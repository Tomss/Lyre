...
export const PdfSplitterModal: React.FC<PdfSplitterModalProps> = ({
  ...
  const toggleInstrumentSelect = (pageIndex: number, instrumentId: string, customName: string) => {
    setSplits(prev => {
      const next = { ...prev };
      if (!instrumentId) {
      ...
      if (instrumentId === '_IGNORE_') {
        next[pageIndex] = { instruments: [{ instrument_id: '_IGNORE_', custom_name: customName }] };
        return next;
      }

      const existing = next[pageIndex] ? { ...next[pageIndex], instruments: [...next[pageIndex].instruments] } : { instruments: [] };
      if (existing.instruments.length === 1 && existing.instruments[0].instrument_id === '_IGNORE_') {
        existing.instruments = [];
      }

      const instIndex = existing.instruments.findIndex(i => i.instrument_id === instrumentId);
      if (instIndex >= 0) {
        existing.instruments.splice(instIndex, 1);
        if (existing.instruments.length === 0) {
           delete next[pageIndex];
        } else {
           next[pageIndex] = existing;
        }
      } else {
        existing.instruments.push({ instrument_id: instrumentId, custom_name: customName });
        next[pageIndex] = existing;
      }
      return next;
    });
  ...
  const updateCustomName = (pageIndex: number, instrumentId: string, newName: string) => {
      setSplits(prev => {
          const next = {...prev};
          if (!next[pageIndex]) return prev;
          const insts = [...next[pageIndex].instruments];
          const instIndex = insts.findIndex(i => i.instrument_id === instrumentId);
          if (instIndex >= 0) {
            insts[instIndex] = { ...insts[instIndex], custom_name: newName };
            next[pageIndex] = { ...next[pageIndex], instruments: insts };
          }
          return next;
      });
  };
              ...
              <div className="flex-1 bg-slate-100 flex flex-col relative overflow-hidden">
                ...
                <div className="bg-white border-t border-slate-200 p-5 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] flex-shrink-0 z-10 w-full">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4 max-h-[140px] overflow-y-auto p-1">
                    <button 
                      onClick={() => toggleInstrumentSelect(currentPage - 1, '', '')} 
                    ...
                    <button 
                      onClick={() => toggleInstrumentSelect(currentPage - 1, '_IGNORE_', 'Pages ignorées')} 
                      className={`px-3 py-2 rounded-xl text-xs font-bold transition flex-shrink-0 ${splits[currentPage - 1]?.instruments?.some(i => i.instrument_id === '_IGNORE_') ? 'bg-red-600 border-red-700 text-white shadow-md ring-2 ring-red-200' : 'bg-white border-slate-200 text-red-600 hover:bg-red-50 border hover:border-red-300'}`}
                    >
                      🚫 Ignorer à partir d'ici (Ne rien générer)
                    </button>
                    {instruments.map(inst => {
                      const isSelected = splits[currentPage - 1]?.instruments?.some(i => i.instrument_id === inst.id);
                      return (
                        <button 
                          key={inst.id} 
                          onClick={() => toggleInstrumentSelect(currentPage - 1, inst.id, inst.name)} 
                  ...
                  {splits[currentPage - 1]?.instruments?.map(inst => {
                    if (inst.instrument_id === '_IGNORE_') return null;
                    const originalInstName = instruments.find(i => i.id === inst.instrument_id)?.name || '';
                    return (
                      <div key={inst.instrument_id} className="flex flex-col md:flex-row md:items-center gap-3 bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 animate-in slide-in-from-bottom-2 mb-2">
                        <label className="text-sm font-semibold text-indigo-900 whitespace-nowrap">Nom pour {originalInstName} :</label>
                        <input 
                          type="text" 
                          value={inst.custom_name}
                          onChange={(e) => updateCustomName(currentPage - 1, inst.instrument_id, e.target.value)}
                          ...
                          placeholder={`Ex: ${originalInstName} 1`}
                        />
                      </div>
                    );
                  })}
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          {splitInfo?.instruments?.some(i => i.instrument_id === '_IGNORE_') ? (
                        ...
                        {splitInfo?.instruments?.some(i => i.instrument_id === '_IGNORE_') ? (
                          <div className="text-xs font-bold text-red-700 bg-red-50 px-2.5 py-1.5 rounded-lg border border-red-100 flex flex-col gap-0.5">
                            <div className="flex items-center"><span className="mr-1.5">🚫</span> {splitInfo.instruments[0].custom_name}</div>
                          ...
                          <div className="text-xs font-bold text-emerald-800 bg-emerald-50 px-2.5 py-1.5 rounded-lg border border-emerald-100 flex flex-col gap-0.5">
                            <div className="flex items-center flex-wrap gap-1">
                              ...
                              {splitInfo.instruments.map(i => i.custom_name).join(', ')}
                            </div>
