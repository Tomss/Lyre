...
import { ChevronDown,  Edit, Trash2, Plus, Music, X, ArrowLeft, Users, ChevronRight, Search  } from "lucide-react";
          ...
          <div className="flex flex-col lg:flex-row gap-6 pt-2 border-t border-slate-100">
            ...
            <div className="lg:border-l lg:pl-6 border-slate-100 flex flex-col justify-start gap-2 border-t lg:border-t-0 pt-4 lg:pt-0 min-w-[200px]">
              ...
              <div className="flex items-center space-x-2">
                <button onClick={expandAllOrchestras} className={`px-4 py-2 rounded-xl transition text-sm font-medium whitespace-nowrap w-full ${expandedOrchestras.size === sortedOrchestras.length ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>Tout déplier</button>
                <button onClick={collapseAllOrchestras} className={`px-4 py-2 rounded-xl transition text-sm font-medium whitespace-nowrap w-full ${expandedOrchestras.size === 0 ? 'bg-slate-600 text-white shadow-md shadow-slate-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>Tout replier</button>
              </div>
            </div>
          </div>
        </div>
