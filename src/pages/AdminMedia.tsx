...
const AdminMedia = () => {
  ...
  <div className="overflow-hidden">
    <div className="divide-y divide-slate-100">
      {items.map(media => (
        <div key={media.id} className="p-4 flex flex-col md:flex-row md:items-center md:justify-between hover:bg-slate-50/80 transition-colors duration-200">
          <div className="flex-1 mb-4 md:mb-0">
            <div className="flex items-center gap-3">
              <p className={`font-semibold ${!media.published ? 'text-slate-500' : 'text-slate-800'}`}>
                {media.title}
              </p>
              {media.published ? (
              ...
              {media.is_featured && (
                <span className="px-2.5 py-0.5 rounded-full bg-amber-50 text-amber-600 border border-amber-100 text-[11px] font-bold tracking-wide flex items-center">
                  <Star size={10} className="mr-1" fill="currentColor" /> Vedette
                </span>
              )}
            </div>
            <p className="text-sm text-slate-500 flex items-center mt-1">
              <Calendar className="w-3 h-3 mr-1.5 opacity-50" />
              {media.media_date ? new Date(media.media_date).toLocaleDateString('fr-FR') : new Date(media.created_at).toLocaleDateString('fr-FR')}
              {media.files && media.files.length > 0 && (
                <span className="ml-3 flex items-center text-indigo-500">
                  <FileText className="w-3 h-3 mr-1" />
                  {media.files.length} fichier(s)
                </span>
              )}
            </p>
          </div>
          <div className="flex items-center space-x-2 flex-shrink-0">
            <button onClick={() => handleEdit(media)} className="p-2 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition-all duration-300 hover:scale-110 shadow-sm" title="Modifier"><Edit size={16} /></button>
            <button onClick={() => confirmDelete(media)} className="p-2 text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-xl transition-all duration-300 hover:scale-110 shadow-sm" title="Supprimer"><Trash2 size={16} /></button>
          </div>
        </div>
      ))}
    </div>
            ...
            <div className="bg-slate-50 rounded-3xl shadow-2xl w-full max-w-3xl flex flex-col overflow-hidden border border-white max-h-[calc(100vh-120px)] animate-in fade-in zoom-in duration-300">
              ...
              <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto p-5 space-y-5 bg-gradient-to-b from-slate-50 to-white">
                ...
                <div className="space-y-4">
                    <div className="flex items-center space-x-2 text-indigo-600 mb-1">
                        <LayoutGrid size={16} />
                        <h3 className="text-xs font-bold uppercase tracking-wider">Type & Date</h3>
                    </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center space-x-2 text-indigo-600 mb-1">
                        <Globe size={16} />
                        <h3 className="text-xs font-bold uppercase tracking-wider">Visibilité & En vedette</h3>
                    </div>

                    <div className="flex flex-col sm:flex-row gap-4 p-4 bg-white rounded-2xl border border-slate-100 shadow-sm transition-all">
                ...
                <div className="space-y-4">
                    <div className="flex items-center space-x-2 text-indigo-600 mb-1">
                        <Users size={16} />
                        <h3 className="text-xs font-bold uppercase tracking-wider">Gestion des fichiers</h3>
                    </div>
                        ...
                        <div>
                            ...
                            <div className="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center hover:border-indigo-400 transition-colors group cursor-pointer relative bg-slate-50/50">
                                ...
                                <div className="flex flex-col items-center">
                                    ...
                                    <p className="text-xs font-semibold text-slate-700">Cliquez ou glissez vos fichiers ici</p>
                                    <p className="text-[10px] text-slate-500 mt-0.5">Photos, Vidéos, Audio ou PDF (Lyrissimots)</p>
                                </div>
                            </div>
                        </div>
